...
<html lang="es">
  <head>
    ...
    <script>
            // Gráfico: Ingresos y Costos por Año/Mes/Día y Sucursal
            var chartIngresosCostos = null;
            ...
            function cargarAniosFiltroIC() {
              // Obtener años de ventas y compras
              $.ajax({
                ...
                success: function(response) {
                  if (response.success && response.data) {
                    ...
                    response.data.forEach(function(d) {
                      if (d.periodo && response.labelType === 'anio') anios.push(d.periodo);
                    });
                    anios = [...new Set(anios)];
                ...
                success: function(response) {
                  if (response.success && response.data) {
                    ...
                    var labels = response.data.map(d => {
                      // Si el periodo es un año válido, mostrarlo, si no, mostrar vacío
                      if (d.periodo && !isNaN(d.periodo)) return d.periodo;
                      return '';
                    });
                    if (labelType === 'mes') {
                    ...
                    if (!chartIngresosCostos) {
                      chartIngresosCostos = echarts.init(document.getElementById('ingresosCostosAnioChart'));
                    }
                    var option = {
                      title: {
                      ...
                      series: [
                        {
                          ...
                          data: ventas,
                          smooth: true,
                          lineStyle: { color: '#f39c12', width: 4 }
                        },
                        {
                          name: 'Inversión',
                          type: 'line',
                          data: compras,
                          smooth: true,
                          lineStyle: { color: '#f1c40f', width: 4 }
                        }
                      ]
                    };
                    chartIngresosCostos.setOption(option);
                    window.addEventListener('resize', function() { chartIngresosCostos.resize(); });
                  }
                }
              });
            }
              ...
              <div class="col-md-12">
                <div class="card border-0 shadow-lg bg-dark-card rounded">
                  <div class="card-header bg-warning text-dark d-flex align-items-center">
                    ...
                    <h5 class="mb-0">Ingresos y Costos por Año</h5>
                  </div>
                  <div class="card-body">
                    <!-- Filtros para el gráfico de ingresos y costos -->
                    <form id="filtrosIngresosCostos" class="row g-2 align-items-end mb-3">
                      <div class="col-md-4">
